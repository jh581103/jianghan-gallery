...
const newThumbName = '旅游_四川博物馆（5）：四川民族文物馆.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 185,
  title: '旅游_四川博物馆（5）：四川民族文物馆',
  url: 'https://weixin.qq.com/sph/A0iWaYMZhC',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '走进四川民族文物馆，感受巴蜀文化',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*185\b/.test(html)) {
  console.log('  ⚠️ id=185 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*185\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_四川博物馆（5）：四川民族文物馆/g) || []).length;
console.log(`  id=185: ${idCount}, title: ${titleCount}`);
