// id=200: 旅游_成都熊猫基地（5）：天鹅湖旁留个影
// 常规录入 (成都熊猫基地系列第 5 集, 录完后江涵决定要不要加 jieziRank)
const fs = require('fs');
...
const newThumbName = '旅游_成都熊猫基地（5）：天鹅湖旁留个影.jpg';
...
if (/"id"\s*:\s*200\b/.test(html)) { console.log('  ⚠️ id=200 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 200,\n` +
  `          "title": "旅游_成都熊猫基地（5）：天鹅湖旁留个影",\n` +
  `          "url": "https://weixin.qq.com/sph/A4huRyCxak",\n` +
  `          "thumbnail": "thumbnails/旅游_成都熊猫基地（5）：天鹅湖旁留个影.jpg",\n` +
  `          "description": "成都熊猫基地天鹅湖旁留个影",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*200\b/g) || []).length;
console.log(`  id=200 计数: ${idCount}`);
