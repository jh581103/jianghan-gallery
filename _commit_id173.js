// id=173: 旅游_成都国庆（2）：合江亭音乐广场音乐会  (特例: 江涵给 description = "合江亭音乐会")
const fs = require('fs');
...
const srcThumb = 'C:\\Users\\HUAWEI\\.minimax\\v2\\assets\\2026\\08\\02\\18-16-38-160-asset_20260802-181638-160_fbd78b86c628_9661fbb6-1000044531.jpg';
const newThumbName = '旅游_成都国庆（2）：合江亭音乐广场音乐会.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 173,
  title: '旅游_成都国庆（2）：合江亭音乐广场音乐会',
  url: 'https://weixin.qq.com/sph/ASykbAIVkS',
...
if (/"id"\s*:\s*173\b/.test(html)) {
  console.log('  ⚠️ id=173 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*173\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_成都国庆（2）：合江亭音乐广场音乐会/g) || []).length;
const descCount = (newHtml.match(/"description": "合江亭音乐会"/g) || []).length;
console.log(`  "id": 173 出现 ${idCount} 次`);
console.log(`  title 出现 ${titleCount} 次`);
console.log(`  description "合江亭音乐会" 出现 ${descCount} 次（应为 1）`);
