// id=154: 旅游_汉中（2）：回到汉人老家留个影
// 改完立刻 node --check 验证 syntax（防 trailing comma 错重演）
const fs = require('fs');
...
const srcThumb = 'C:\\Users\\HUAWEI\\.minimax\\v2\\assets\\2026\\08\\02\\10-04-13-460-asset_20260802-100413-460_ca520e7f91c1_cd327af8-1000044427.jpg';
const newThumbName = '旅游_汉中（2）：回到汉人老家留个影.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 154,
  title: '旅游_汉中（2）：回到汉人老家留个影',
  url: 'https://weixin.qq.com/sph/AhaWEjowzc',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '汉中（2）回到汉人老家留个影纪念。',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*154\b/.test(html)) {
  console.log('  ⚠️ id=154 已存在，跳过');
} else {
  // 关键：用 JSON.stringify 输出后**不**拼任何额外字符（包括 ;）
  // 前导缩进对齐到原来 6 空格的格式
...
console.log('\n=== 步骤 3: syntax 验证（防 trailing comma 错重演）===');
...
const idCount = (newHtml.match(/"id"\s*:\s*154\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_汉中（2）：回到汉人老家留个影/g) || []).length;
console.log(`  "id": 154 出现 ${idCount} 次`);
console.log(`  title 出现 ${titleCount} 次（应是 2：works + thumb）`);
