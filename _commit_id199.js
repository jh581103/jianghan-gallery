// id=199: 旅游_成都熊猫基地（4）：今日参观人超多
// 序号: 江涵确认 (3) 已经被睡美熊猫占, 这条录为 (4)
const fs = require('fs');
...
const newThumbName = '旅游_成都熊猫基地（4）：今日参观人超多.jpg';
...
if (/"id"\s*:\s*199\b/.test(html)) { console.log('  ⚠️ id=199 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 199,\n` +
  `          "title": "旅游_成都熊猫基地（4）：今日参观人超多",\n` +
  `          "url": "https://weixin.qq.com/sph/Akuhd48h0Y",\n` +
  `          "thumbnail": "thumbnails/旅游_成都熊猫基地（4）：今日参观人超多.jpg",\n` +
  `          "description": "成都熊猫基地今日参观人超多，入口处人山人海",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*199\b/g) || []).length;
console.log(`  id=199 计数: ${idCount}`);
