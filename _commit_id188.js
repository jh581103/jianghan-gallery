...
const newThumbName = '旅游_成都非遗节文艺表演（2）.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 188,
  title: '旅游_成都非遗节文艺表演（2）',
  url: 'https://weixin.qq.com/sph/A0CJUT0hiF',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '成都非遗节中国传统器乐周武侯区主题活动',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*188\b/.test(html)) {
  console.log('  ⚠️ id=188 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*188\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_成都非遗节文艺表演（2）/g) || []).length;
console.log(`  id=188: ${idCount}, title: ${titleCount}`);
