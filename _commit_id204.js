...
const newThumbName = '旅游_半亩方塘（2）：酒店院落实拍.jpg';
...
if (/"id"\s*:\s*204\b/.test(html)) { console.log('  ⚠️ id=204 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 204,\n` +
  `          "title": "旅游_半亩方塘（2）：酒店院落实拍",\n` +
  `          "url": "https://weixin.qq.com/sph/AJDbAGwPvm",\n` +
  `          "thumbnail": "thumbnails/旅游_半亩方塘（2）：酒店院落实拍.jpg",\n` +
  `          "description": "温江半亩方塘酒店院落实拍",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*204\b/g) || []).length;
console.log(`  id=204 计数: ${idCount}`);
