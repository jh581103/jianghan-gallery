// id=158: 旅游_汉中（6）：驶出秦岭，驰入平川；柏油云絮，隙露蓝天。
const fs = require('fs');
...
const srcThumb = 'C:\\Users\\HUAWEI\\.minimax\\v2\\assets\\2026\\08\\02\\11-40-53-074-asset_20260802-114053-074_043c8bcacdb8_88136d40-1000044440.jpg';
const newThumbName = '旅游_汉中（6）：驶出秦岭，驰入平川；柏油云絮，隙露蓝天。.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 158,
  title: '旅游_汉中（6）：驶出秦岭，驰入平川；柏油云絮，隙露蓝天。',
  url: 'https://weixin.qq.com/sph/AdF09iZTkk',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '柏油云絮，隙露蓝天。',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*158\b/.test(html)) {
  console.log('  ⚠️ id=158 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*158\b/g) || []).length;
const titleCount = (newHtml.match(/驶出秦岭，驰入平川/g) || []).length;
console.log(`  "id": 158 出现 ${idCount} 次`);
console.log(`  title 部分出现 ${titleCount} 次（应是 2）`);
