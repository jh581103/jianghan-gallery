...
const newThumbName = '旅游_大邑朗星家庭农场实拍.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 193,
  title: '旅游_大邑朗星家庭农场实拍',
  url: 'https://weixin.qq.com/sph/ADkBvekVWr',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '大邑朗星家庭农场，稻香渔歌主题园区',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*193\b/.test(html)) {
  console.log('  ⚠️ id=193 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*193\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_大邑朗星家庭农场实拍/g) || []).length;
const dateCount = (newHtml.match(/"publishDate":\s*"2026-08-03"/g) || []).length;
console.log(`  id=193: ${idCount}, title: ${titleCount}, date 2026-08-03: ${dateCount}`);
