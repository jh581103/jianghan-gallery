...
const newThumbName = '旅游_街子古镇（2）：艾草红汤温泉专场独泡.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 190,
  title: '旅游_街子古镇（2）：艾草红汤温泉专场独泡',
  url: 'https://weixin.qq.com/sph/AAZyzY5abU',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '街子古镇艾草红汤温泉专场独泡，解乏放松',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*190\b/.test(html)) {
  console.log('  ⚠️ id=190 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*190\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_街子古镇（2）：艾草红汤温泉专场独泡/g) || []).length;
const dateCount = (newHtml.match(/"publishDate":\s*"2026-08-03"/g) || []).length;
console.log(`  id=190: ${idCount}, title: ${titleCount}, date 2026-08-03: ${dateCount}`);
