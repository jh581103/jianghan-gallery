// id=207: 旅游_半亩方塘（5）：方塘院里开心跳
// 常规录入 (半亩方塘系列第 5 集, 最后 1 集, 不加 jieziRank)
const fs = require('fs');
...
const newThumbName = '旅游_半亩方塘（5）：方塘院里开心跳.jpg';
...
if (/"id"\s*:\s*207\b/.test(html)) { console.log('  ⚠️ id=207 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 207,\n` +
  `          "title": "旅游_半亩方塘（5）：方塘院里开心跳",\n` +
  `          "url": "https://weixin.qq.com/sph/AIwjDDQ6Sh",\n` +
  `          "thumbnail": "thumbnails/旅游_半亩方塘（5）：方塘院里开心跳.jpg",\n` +
  `          "description": "温江半亩方塘酒店方塘院里开心跳",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*207\b/g) || []).length;
console.log(`  id=207 计数: ${idCount}`);
