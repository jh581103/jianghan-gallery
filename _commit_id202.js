// id=202: 旅游_成都马拉松（2）：坚持下去的勇气
// 常规录入 (成都马拉松系列第 2 集, 2 集录完不加 jieziRank)
const fs = require('fs');
...
const newThumbName = '旅游_成都马拉松（2）：坚持下去的勇气.jpg';
...
if (/"id"\s*:\s*202\b/.test(html)) { console.log('  ⚠️ id=202 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 202,\n` +
  `          "title": "旅游_成都马拉松（2）：坚持下去的勇气",\n` +
  `          "url": "https://weixin.qq.com/sph/A2kfX4KntC",\n` +
  `          "thumbnail": "thumbnails/旅游_成都马拉松（2）：坚持下去的勇气.jpg",\n` +
  `          "description": "2023 成都马拉松坚持下去的勇气，老当益壮",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*202\b/g) || []).length;
console.log(`  id=202 计数: ${idCount}`);
