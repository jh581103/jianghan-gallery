...
if (works.find(w => w.id === 212)) {
  console.log('❌ id=212 已存在，不能重复添加');
  process.exit(1);
}

const newWork = {
  id: 212,
  title: '旅游_成都天府广场3：花开盛世',
  url: 'https://weixin.qq.com/sph/As2WetLWFG',
  thumbnail: 'thumbnails/旅游_成都天府广场3：花开盛世.jpg',
  description: '成都天府广场巨型牡丹花雕塑，花开盛世主题。',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
console.log('✓ index.html 已写入新作品 id=212');
...
const w212 = works2.find(w => w.id === 212);
if (!w212) { console.log('❌ 验证失败：id=212 未找到'); process.exit(1); }
console.log('✓ 验证：id=212 已写入：', JSON.stringify(w212, null, 2));
