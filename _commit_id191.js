...
const newThumbName = '旅游_蓝天碧水九眼桥府南河.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 191,
  title: '旅游_蓝天碧水九眼桥府南河',
  url: 'https://weixin.qq.com/sph/AZPfhM7FmM',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '成都九眼桥府南河畔，蓝天碧水相映',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*191\b/.test(html)) {
  console.log('  ⚠️ id=191 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*191\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_蓝天碧水九眼桥府南河/g) || []).length;
const dateCount = (newHtml.match(/"publishDate":\s*"2026-08-03"/g) || []).length;
console.log(`  id=191: ${idCount}, title: ${titleCount}, date 2026-08-03: ${dateCount}`);
