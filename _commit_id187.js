...
const newThumbName = '旅游_成都非遗节文艺表演（1）.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 187,
  title: '旅游_成都非遗节文艺表演（1）',
  url: 'https://weixin.qq.com/sph/A9tSLTO5XB',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '2023 第8届成都非遗节文艺表演现场',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*187\b/.test(html)) {
  console.log('  ⚠️ id=187 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*187\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_成都非遗节文艺表演（1）/g) || []).length;
console.log(`  id=187: ${idCount}, title: ${titleCount}`);
