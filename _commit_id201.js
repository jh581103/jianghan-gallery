// id=201: 旅游_成都马拉松（1）：大场面
// 常规录入 (新系列"成都马拉松"第 1 集)
const fs = require('fs');
...
const newThumbName = '旅游_成都马拉松（1）：大场面.jpg';
...
if (/"id"\s*:\s*201\b/.test(html)) { console.log('  ⚠️ id=201 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 201,\n` +
  `          "title": "旅游_成都马拉松（1）：大场面",\n` +
  `          "url": "https://weixin.qq.com/sph/AU1DTJ9hSD",\n` +
  `          "thumbnail": "thumbnails/旅游_成都马拉松（1）：大场面.jpg",\n` +
  `          "description": "成都马拉松大场面，古牌坊下人潮涌动",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*201\b/g) || []).length;
console.log(`  id=201 计数: ${idCount}`);
