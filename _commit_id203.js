// id=203: 旅游_半亩方糖（1）：温江悠然半亩方塘酒店实拍
// 常规录入 (新系列"半亩方糖/方塘"第 1 集)
// 提醒: 用户写"半亩方糖", 截图配文"半亩方塘" - 同音字, 复述时问
const fs = require('fs');
...
const newThumbName = '旅游_半亩方糖（1）：温江悠然半亩方塘酒店实拍.jpg';
...
if (/"id"\s*:\s*203\b/.test(html)) { console.log('  ⚠️ id=203 已存在'); process.exit(1); }

const newObjText =
  '\n      {\n' +
  `          "id": 203,\n` +
  `          "title": "旅游_半亩方糖（1）：温江悠然半亩方塘酒店实拍",\n` +
  `          "url": "https://weixin.qq.com/sph/AW9ekm1Usr",\n` +
  `          "thumbnail": "thumbnails/旅游_半亩方糖（1）：温江悠然半亩方塘酒店实拍.jpg",\n` +
  `          "description": "温江悠然半亩方塘酒店房间实拍",\n` +
  `          "tags": [\n` +
  `            "江涵原创",\n` +
  `            "旅游",\n` +
  `            "视频"\n` +
...
const idCount = (newHtml.match(/"id"\s*:\s*203\b/g) || []).length;
console.log(`  id=203 计数: ${idCount}`);
