...
const newThumbName = '旅游_稻香村农家待宰羊.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 192,
  title: '旅游_稻香村农家待宰羊',
  url: 'https://weixin.qq.com/sph/APB8drnGgd',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '稻香村农家竹棚里待宰的羊',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*192\b/.test(html)) {
  console.log('  ⚠️ id=192 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*192\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_稻香村农家待宰羊/g) || []).length;
const dateCount = (newHtml.match(/"publishDate":\s*"2026-08-03"/g) || []).length;
console.log(`  id=192: ${idCount}, title: ${titleCount}, date 2026-08-03: ${dateCount}`);
