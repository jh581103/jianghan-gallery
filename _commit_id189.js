// id=189: 旅游_街子古镇（1）斯维登温泉酒店实拍
// 新规则 (2026-08-03): publishDate = 2026-08-03 (今天)
const fs = require('fs');
...
const newThumbName = '旅游_街子古镇（1）斯维登温泉酒店实拍.jpg';
const destThumb = path.join(thumbsDir, newThumbName);

const newWork = {
  id: 189,
  title: '旅游_街子古镇（1）斯维登温泉酒店实拍',
  url: 'https://weixin.qq.com/sph/A4G0Q3yoVY',
  thumbnail: 'thumbnails/' + newThumbName,
  description: '街子古镇斯维登温泉酒店，古色古香',
  tags: ['江涵原创', '旅游', '视频'],
  medium: '视频',
  platform: '视频号',
...
if (/"id"\s*:\s*189\b/.test(html)) {
  console.log('  ⚠️ id=189 已存在，跳过');
} else {
...
const idCount = (newHtml.match(/"id"\s*:\s*189\b/g) || []).length;
const titleCount = (newHtml.match(/旅游_街子古镇（1）斯维登温泉酒店实拍/g) || []).length;
const dateCount = (newHtml.match(/"publishDate":\s*"2026-08-03"/g) || []).length;
console.log(`  id=189: ${idCount}, title: ${titleCount}, date 2026-08-03: ${dateCount}`);
